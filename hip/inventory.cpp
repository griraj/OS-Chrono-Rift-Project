/*
 * inventory.cpp
 * Contiguous-fit inventory allocator implementation.
 */
...
int inv_find_fit(const Inventory& inv, int size)
{
    for (int i = 0; i <= INVENTORY_SLOTS - size; ++i)
        if (free_run_length(inv, i) >= size) return i;
    return -1;
}
...
{
    if (start < 0 || start >= INVENTORY_SLOTS) return WPN_NONE;
    WeaponID id = inv.slots[start];
    if (id == WPN_NONE) return WPN_NONE;
...
{
    if (slot < 0 || slot >= INVENTORY_SLOTS) return -1;
    WeaponID id = inv.slots[slot];
    if (id == WPN_NONE) return -1;
    int s = slot;
    while (s > 0 && inv.slots[s - 1] == id) --s;
    return s;
}
...
bool inv_pickup(Entity& e, WeaponID id)
{
    ...
    if (need <= 0 || need > INVENTORY_SLOTS) return false;
    ...
    /* Need to swap out weapons to LTS.
     * Greedy: scan left-to-right, swap out each weapon one at a time
     * until a contiguous gap of `need` slots exists.
     * This guarantees minimum swaps (only swap what's blocking). */
    Inventory   tmp_inv = e.inventory;
    LongTermStorage tmp_lts = e.lts;

    for (int i = 0; i < INVENTORY_SLOTS; ++i) {
        if (!is_first_slot(tmp_inv, i)) continue;
    ...
    return false; /* weapon larger than entire inventory — impossible in our table */
}
...
bool inv_swap_in(Entity& e, WeaponID id)
{
    ...
    for (int i = 0; i < e.lts.count; ++i)
        if (e.lts.weapons[i] == id) { found = i; break; }
...
{
    for (int i = 0; i < INVENTORY_SLOTS; ++i)
        if (e.inventory.slots[i] == id) return true;
    return false;
}
