...
static void Corners(sf::RenderTarget &r, float x, float y, float w, float h, float sz = 10.f)
{
    ...
    seg(x, y + h - 2, sz, 2);
    seg(x, y + h - sz, 2, sz);
    seg(x + w - sz, y + h - 2, sz, 2);
    seg(x + w - 2, y + h - sz, 2, sz);
}

static void Scanlines(sf::RenderTarget &r)
{
    sf::RectangleShape l({(float)WW, 1.f});
    l.setFillColor(SCAN);
    for (unsigned y = 0; y < WH; y += 4)
    {
...
{
    sf::RectangleShape l({(float)WW, 1.f});
    l.setFillColor({20, 30, 60, 30});
    for (unsigned y = 0; y < WH; y += 32)
    {
        l.setPosition(0, (float)y);
        r.draw(l);
    }
    l.setSize({1.f, (float)WH});
    for (unsigned x = 0; x < WW; x += 32)
    {
        l.setPosition((float)x, 0);
        r.draw(l);
    }
}
    ...
    while (win.isOpen() && !g_quit)
    {
        ...
        while (win.pollEvent(ev))
        {
                ...
                ev.mouseButton.button == sf::Mouse::Left)
            {
                ...
                if (resume_b.contains(msc))
                    return false;
                if (quit_b.contains(msc))
                {
        ...
        sf::RectangleShape rl({300.f, 1.f});
        rl.setPosition(WW / 2.f - 150.f, WH / 2.f - 32.f);
        rl.setFillColor(BDR);
        win.draw(rl);
...
struct HipUI
{
    ...
    void begin_turn(int p)
    {
        ...
        for (int s = 0; s < INVENTORY_SLOTS; s++)
            if (me.inventory.slots[s] != WPN_NONE)
            {
                hi = true;
                break;
            }
        ...
        if (can_ult)
            add("7  ULTIMATE", "Solar Core + Lunar Blade", ULT);
        ...
        for (int i = np; i < nte; i++)
            if (ents[i].alive)
            {
                enemy_btns.push_back({{400.f, ey, WW - 420.f, 38.f}, i});
                ey += 44.f;
            }
        ...
        for (int s = 0; s < INVENTORY_SLOTS; s++)
        {
            ...
            if (w == WPN_NONE)
                continue;
            if (inv_first_slot_of(me.inventory, s) != s)
                continue;
            wpn_btns.push_back({{30.f, wy, 340.f, 42.f}, s, w});
            wy += 48.f;
            for (int k = s; k < INVENTORY_SLOTS && me.inventory.slots[k] == w; k++)
                vis[k] = true;
        }
        ...
        {
            lts_btns.push_back({{30.f, ly, 340.f, 42.f}, i, me.lts.weapons[i]});
            ly += 48.f;
        }
    }
    ...
    bool handle_click(sf::Vector2f ms, Action &out)
    {
        if (step == Step::ACTION)
        {
            for (int i = 0; i < (int)action_btns.size(); i++)
            {
                ...
                if (l[0] == '0')
                {
                    kill(gs->arbiter_pid, SIGTERM);
                    g_quit = 1;
                    out = {ActionType::SKIP, -1, -1, WPN_NONE};
                    return true;
                }
                if (l.find("Strike") != std::string::npos)
                {
                    step = Step::ENEMY;
                    status = "Click an enemy to strike";
                }
                else if (l.find("Exhaust") != std::string::npos)
                {
                    step = Step::ENEMY;
                    status = "Click an enemy to exhaust";
                }
                else if (l.find("Weapon") != std::string::npos)
                {
                    step = Step::WEAPON;
                    status = "Click a weapon to use";
                }
                else if (l.find("Swap") != std::string::npos)
                {
                    step = Step::LTS;
                    status = "Click weapon from storage";
                }
                else if (l.find("Heal") != std::string::npos)
                {
                    out = {ActionType::HEAL, -1, -1, WPN_NONE};
                    return true;
                }
                else if (l.find("Skip") != std::string::npos)
                {
                    out = {ActionType::SKIP, -1, -1, WPN_NONE};
                    return true;
                }
                else if (l.find("ULTIMATE") != std::string::npos)
                {
                    out = {ActionType::ULTIMATE, -1, -1, WPN_NONE};
                    return true;
                }
                return false;
            }
        ...
        if (step == Step::ENEMY)
        {
            for (auto &eb : enemy_btns)
            {
                ...
                if (sel_act >= 0)
                {
                    ...
                    if (l.find("Exhaust") != std::string::npos)
                        at = ActionType::EXHAUST;
                    else if (l.find("Weapon") != std::string::npos)
                        at = ActionType::USE_WEAPON;
                }
                out = {at, eb.idx, sel_wpn_slot, WPN_NONE};
                return true;
            }
        }
        if (step == Step::WEAPON)
        {
            for (auto &wb : wpn_btns)
            {
                if (!wb.b.contains(ms))
                    continue;
                sel_wpn_slot = wb.slot;
                step = Step::ENEMY;
                status = "Now click a target enemy";
                return false;
            }
        }
        if (step == Step::LTS)
        {
            for (auto &lb : lts_btns)
            {
                if (!lb.b.contains(ms))
                    continue;
                out = {ActionType::SWAP_IN, -1, -1, lb.wid};
                return true;
            }
        }
        return false;
    ...
    void draw_wait(sf::RenderWindow &win)
    {
        ...
        char kb[32];
        snprintf(kb, sizeof(kb), "Kills: %d / %d", kills, MAX_ENEMIES_KILL);
        Tcx(win, T(font, kb, 13, sf::Color{80, 200, 80}), WW / 2.f, 48.f);

        float col_w = (WW - 40.f) / 2.f;

        auto ph = T(font, "PLAYERS", 12, sf::Color{100, 120, 180});
        ph.setPosition(24.f, 74.f);
        win.draw(ph);
        float py = 96.f;
        for (int i = 0; i < np; i++)
        {
            ...
            bool active = (ct == i);
            sf::Color nc = !e.alive ? sf::Color{80, 40, 40} : active ? GOLD
                                                                     : WHITE;
            char ln[80];
            snprintf(ln, sizeof(ln), "%s%s  HP:%d/%d  ST:%.0f",
                     e.name, active ? " <" : "", e.hp, e.max_hp, e.stamina);
            auto t = T(font, ln, 13, nc);
            t.setPosition(24.f, py);
            win.draw(t);

            float bw = col_w - 20.f, fill = e.max_hp > 0 ? (float)e.hp / e.max_hp * bw : 0.f;
            sf::RectangleShape bg({bw, 5.f});
            bg.setPosition(24.f, py + 17.f);
            bg.setFillColor({30, 40, 30});
            win.draw(bg);
            sf::RectangleShape bar({fill, 5.f});
            bar.setPosition(24.f, py + 17.f);
            bar.setFillColor({60, 200, 80});
            win.draw(bar);

            float sf2 = e.max_stamina > 0 ? e.stamina / e.max_stamina * bw : 0.f;
            sf::RectangleShape sbg({bw, 3.f});
            sbg.setPosition(24.f, py + 24.f);
            sbg.setFillColor({20, 20, 50});
            win.draw(sbg);
            sf::RectangleShape sbar({sf2, 3.f});
            sbar.setPosition(24.f, py + 24.f);
            sbar.setFillColor({60, 140, 255});
            win.draw(sbar);
            py += 36.f;
        }

        auto eh = T(font, "ENEMIES", 12, sf::Color{180, 80, 80});
        eh.setPosition(WW / 2.f + 4.f, 74.f);
        win.draw(eh);
        float ey = 96.f;
        for (int i = 0; i < ne; i++)
        {
            ...
            bool active = (ct == np + i);
            sf::Color nc = !e.alive ? sf::Color{60, 30, 30} : active ? sf::Color{255, 160, 60}
                                                                     : sf::Color{200, 160, 160};
            char ln[80];
            snprintf(ln, sizeof(ln), "%s%s  HP:%d  ST:%.0f",
                     e.name, active ? " <" : "", e.hp, e.stamina);
            auto t = T(font, ln, 13, nc);
            t.setPosition(WW / 2.f + 4.f, ey);
            win.draw(t);
            float bw = col_w - 20.f, fill = e.max_hp > 0 ? (float)e.hp / e.max_hp * bw : 0.f;
            sf::RectangleShape bg({bw, 5.f});
            bg.setPosition(WW / 2.f + 4.f, ey + 17.f);
            bg.setFillColor({40, 20, 20});
            win.draw(bg);
            sf::RectangleShape bar({fill, 5.f});
            bar.setPosition(WW / 2.f + 4.f, ey + 17.f);
            bar.setFillColor({220, 60, 60});
            win.draw(bar);
            ey += 36.f;
        }
    ...
    void draw_picking(sf::RenderWindow &win)
    {
...

        {
            ...
            ti.setPosition(20.f, 10.f);
            win.draw(ti);
            const char *ph =
                step == Step::ACTION ? "Choose Action" : step == Step::ENEMY ? "Choose Target"
                                                     : step == Step::WEAPON  ? "Choose Weapon"
                                                                             : "Choose from Storage";
            auto pt = T(font, ph, 14, ACCENT);
            sf::FloatRect pb = pt.getLocalBounds();
            pt.setPosition(WW - pb.width - 20.f, 16.f);
            win.draw(pt);

            auto ph2 = T(font, "P = menu", 11, sf::Color{60, 70, 90});
            sf::FloatRect pb2 = ph2.getLocalBounds();
            ph2.setPosition(WW - pb2.width - 20.f, 36.f);
            win.draw(ph2);
            sf::RectangleShape rule({(float)WW - 40.f, 1.f});
            rule.setPosition(20.f, 50.f);
            rule.setFillColor(BDR);
            win.draw(rule);
        }
        ...
        {
            auto nm = T(font, me.name, 16, WHITE, true);
            nm.setPosition(34.f, 66.f);
            win.draw(nm);
            std::ostringstream hs, ss;
            hs << me.hp << "/" << me.max_hp;
            ss << (int)me.stamina << "/" << me.max_stamina;
            auto hl = T(font, "HP", 11, DIM);
            hl.setPosition(34.f, 90.f);
            win.draw(hl);
            Bar(win, 60.f, 92.f, 280.f, 14.f, (float)me.hp, (float)me.max_hp, HP_P);
            auto hv = T(font, hs.str(), 11, WHITE);
            hv.setPosition(348.f, 90.f);
            win.draw(hv);
            auto sl = T(font, "ST", 11, DIM);
            sl.setPosition(34.f, 114.f);
            win.draw(sl);
            Bar(win, 60.f, 116.f, 280.f, 14.f, me.stamina, (float)me.max_stamina, STAM);
            auto sv = T(font, ss.str(), 11, WHITE);
            sv.setPosition(348.f, 114.f);
            win.draw(sv);
            std::ostringstream ds;
            ds << "DMG: " << me.damage << "   SPD: " << me.speed;
            auto dv = T(font, ds.str(), 13, DIM);
            dv.setPosition(34.f, 138.f);
            win.draw(dv);

            float ix = 450.f, iy = 68.f;
            auto il = T(font, "INVENTORY", 11, DIM);
            il.setPosition(ix, iy);
            win.draw(il);
            iy += 16.f;
            ...
            for (int s = 0; s < INVENTORY_SLOTS; s++)
            {
                ...
                ws << "[" << s << "] " << weapon_def(w).name;
                auto wt = T(font, ws.str(), 12, ACCENT);
                wt.setPosition(ix, iy);
                win.draw(wt);
                iy += 15.f;
                any = true;
                for (int k = s; k < INVENTORY_SLOTS && me.inventory.slots[k] == w; k++)
            ...
            if (!any)
            {
                auto et = T(font, "(empty)", 12, DIM);
                et.setPosition(ix, iy);
                win.draw(et);
            }
            if (can_ult)
            {
                auto ut = T(font, "*** ULTIMATE AVAILABLE ***", 13, ULT, true);
                ut.setPosition(34.f, 158.f);
                win.draw(ut);
            }
        }
        ...
        if (step == Step::ACTION)
        {
            ...
            for (auto &b : action_btns)
            {
                ...
                sf::Color tc = !b.enabled ? DIM : hov ? WHITE
                                                      : sf::Color{160, 170, 190};
                auto lt = T(font, b.lbl, 15, tc, hov);
                lt.setPosition(b.b.left + 14.f, b.b.top + 7.f);
                win.draw(lt);
                if (!b.sub.empty())
                {
                    auto st = T(font, b.sub, 11, hov ? ACCENT : DIM);
                    st.setPosition(b.b.left + 14.f, b.b.top + b.b.height - 18.f);
                    win.draw(st);
                }
            }
        }
        else if (step == Step::WEAPON)
        {
            ...
            for (auto &wb : wpn_btns)
            {
                ...
                wt.setPosition(wb.b.left + 8.f, wb.b.top + 12.f);
                win.draw(wt);
            }
        }
        else if (step == Step::LTS)
        {
            ...
            for (auto &lb : lts_btns)
            {
                ...
                lt.setPosition(lb.b.left + 8.f, lb.b.top + 12.f);
                win.draw(lt);
            }
        }
        else
        {
            ...
            lhdr.setPosition(34.f, 220.f);
            win.draw(lhdr);
            auto arr = T(font, ">>>", 22, RED, true);
            arr.setPosition(34.f, 260.f);
            win.draw(arr);
            if (sel_act >= 0)
            {
                auto ab = T(font, "Action: " + action_btns[sel_act].lbl, 13, ACCENT);
                ab.setPosition(34.f, 300.f);
                win.draw(ab);
            }
        }
        ...
        for (int i = np; i < nte; i++)
        {
            ...
            for (auto &eb : enemy_btns)
                if (eb.idx == i)
                {
                    ...
                    hov = clickable && eb.b.contains(ms);
                    break;
                }
            ...
            if (en.alive)
            {
                ...
                hs << en.hp << "/" << en.max_hp;
                auto hv = T(font, hs.str(), 10, DIM);
                hv.setPosition(524.f, ey + 20.f);
                win.draw(hv);
                Bar(win, 570.f, ey + 22.f, 80.f, 7.f, en.stamina, (float)en.max_stamina, STAM);
                std::ostringstream ss;
                ss << "ST " << (int)en.stamina;
                auto sv = T(font, ss.str(), 10, DIM);
                sv.setPosition(656.f, ey + 20.f);
                win.draw(sv);
            }
            ey += 44.f;
        }
...
static void *player_thread(void *arg_)
{
    ...
    while (!g_quit)
    {
        ...
        if (g_quit)
            break;
        if (gs->current_turn != pidx || !gs->turn_ready || !gs->entities[pidx].alive)
            continue;
...
int main(int argc, char *argv[])
{
    if (argc < 3)
    {
        fprintf(stderr, "Usage: hip <roll_no> <num_players>\n");
        return 1;
    }
        ...
        if (gs->pending_drop_ready && !gs->pending_drop_done)
        {
            ...
            if (dfor >= 0 && dfor < g_num_players)
            {
                ...
                while (dwin.isOpen())
                {
                    ...
                    yt.setPosition(yes_b.left + yes_b.width / 2.f, yes_b.top + 12.f);
                    dwin.draw(yt);
                    ...
                    nt2.setPosition(no_b.left + no_b.width / 2.f, no_b.top + 12.f);
                    dwin.draw(nt2);
                    dwin.display();
                }
            }
