...
static int scheduler_tick()
{
    ...
    for (int i = 0; i < gs->total_entities; ++i)
    {
        ...
        if (!e.alive || e.stunned)
            continue;
        if (g_ultimate && e.type == EntityType::ENEMY)
            continue;
...
static void arbiter_main_loop()
{
    ...
    while (!check_end_conditions())
    {
        ...
        if ((act.type == ActionType::STRIKE || act.type == ActionType::USE_WEAPON) && act.target_idx >= 0 && gs->entities[act.target_idx].alive)
        {
            unsigned tmp = g_seed;
        ...
        if (ultimate_triggered)
            trigger_ultimate(ready);
        if (stun_triggered)
            deliver_stun(act.target_idx);
...
static void *render_thread_fn(void *)
{
...

    {
               ...
               splash_clk.getElapsedTime().asSeconds() < SPLASH_SECS)
        {
            ...
            if (t < 0.6f)
                alpha = t / 0.6f;
            else if (t > SPLASH_SECS - 0.5f)
                alpha = (SPLASH_SECS - t) / 0.5f;
            if (alpha < 0.f)
                alpha = 0.f;
            if (alpha > 1.f)
                alpha = 1.f;
    ...
    while (window.isOpen() && !g_quit && !g_render_done)
    {
...

        {
            const char *phase_str =
                snap.phase == GamePhase::ULTIMATE_PAUSE ? "  *** ULTIMATE PAUSE ***" : snap.phase == GamePhase::GAME_OVER ? "  GAME OVER"
                                                                                                                          : "  RUNNING";
            sf::Color phase_col =
                snap.phase == GamePhase::ULTIMATE_PAUSE ? C_ULTIMATE : snap.phase == GamePhase::GAME_OVER ? C_HP_ENEMY
                                                                                                          : C_HP_PLAYER;
        ...
        for (int l = 0; l < max_lines; ++l)
        {
            ...
            if (line_str.find("ULTIMATE") != std::string::npos)
                ...
                lc = C_STUN;
            else if (line_str.find("DEADLOCK") != std::string::npos)
                lc = C_HP_ENEMY;
            else if (line_str.find("DEFEAT") != std::string::npos)
                lc = C_HP_ENEMY;
            else if (line_str.find("VICTORY") != std::string::npos)
                lc = C_ACTIVE;

            if (line_str.size() > 42)
                line_str = line_str.substr(0, 39) + "...";
        ...
        if (snap.phase == GamePhase::GAME_OVER)
        {
            ...
            const char *msg =
                snap.result == GameResult::WIN ? "VICTORY!" : snap.result == GameResult::LOSE ? "DEFEAT!"
                                                                                              : "GAME QUIT";
